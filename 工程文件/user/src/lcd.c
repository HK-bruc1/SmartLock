...
void LCD_init(void)
{
	...
	//清屏，防止之前有遗留数据导致页面初始化时显示其他的东西
	//先清除再亮灯就不会看到屏幕上残余数据了，还是有刷新为黑色的效果
    //LCD_clear(BLACK);软件处理掉，比如恢复出产设置时，先关灯再把界面设置为主界面即可，就看不到明显的画面切换了
	
	//开背光，高电平有效。
	//LCD_LED_ON;
	//定时器3的PWM输出初始化,PB1,控制LCD屏幕亮度，初始占空比为100,有效电平为高
	TIM3_ch4_LCD_Pwm();
}
