...
void task(void){
    ...
    //如果进入主循环之前没有连接过WiFi，ESP32是不会重连的。可以加一个初始化是否成功连接上WiFi的标志位
    //开启周期性检查标志位被置位后才进行周期性检查，进入主循环后，标志位一直不会变，主循环会一直检查WiFi状态
    if(tim9_count[5]>=60000 && wifi_check_start==1){
        //检查完MQTT后再清零
        tim9_count[5] = 0;
        printf("周期性检查WiFi状态\r\n");
        //清理接收缓存，主循环中的数据在经过解析后都会清理不需要自己清理
        //clean_buff();
