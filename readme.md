...
```
1.把中断中的数据解析操作拿出来改用标志位，放在主函数中循环
2.EPS32返回数据格式不只一个换行符，使用它判断数据帧传输完成不太好，直接在后面加\0即可，直接使用函数判断
3.定时任务也拿出来单独封装成函数，1ms定时中断不适合太时间操作，拿出来后数据解析就正常了
4.WiFi断开后可以自动尝试连接，连接后因为MQTT在while1前面所以直接软件复位
5.改进：把MQTT放在主函数中，使用标志位判断WiFi是否重连，重连后就再连接MQTT
```
